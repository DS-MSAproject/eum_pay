...
## 4) 호환성 특이사항 (기존 DTO 영향 없음)

- 이번 라운드에서 `reviewserver` API는 **기존 DTO 필드 구조를 변경하지 않았습니다**.
- 프론트엔드 기존 요청/응답 스키마(`data`, `files`, `publicId`, `reviewMedias` 등)는 그대로 유지됩니다.
- 권한/헤더 정책 및 내부 검증 로직이 조정되더라도, DTO 계약은 유지되어 **프론트 코드 수정이 필요하지 않도록** 설계했습니다.
